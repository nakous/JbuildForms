...
<%-- Component to show a single success message according to messageCode parameter. 
...
Example usage:
<b:success messageCode="message.code" />
--%>

<c:set var="hidden" value="${(empty visible) ? '' : (visible ? '' : 'hidden')}" />

<div id="${alertId}" class="alert alert-success alert-dismissible fade in ${hidden}" role="alert">
	<div class="alert-icon">
		<span class="hr-icon__check hr-icon-38 text-success"></span>
	</div>
	<div class="alert-body">
		...
		<strong><spring:message code="system.messages.success" /></strong><br />
